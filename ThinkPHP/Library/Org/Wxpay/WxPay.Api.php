...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(C('Wx.notify_url'));//异步通知url
		}
		
		$inputObj->SetAppid(C('Wx.AppID'));//公众账号ID
		$inputObj->SetMch_id(C('Wx.mch_id'));//商户号
	...
	{
		//如果不需要上报数据
		if(C('Wx.REPORT_LEVENL') == 0){
			return;
		} 
		//如果仅失败上报
		if(C('Wx.REPORT_LEVENL') == 1 &&
